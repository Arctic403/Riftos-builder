...
node -e '
  ...
  if (phase?.schema !== "rift-memory-n2-phase-authority-v1" ||
      phase?.programStatus !== "N2.0-N2.6 PROMOTED / N2-M1 + N2-M2 + N2-M3 PROMOTED; N2.7-N2.8 SOURCE IMPLEMENTED / N2-M4 SOURCE IMPLEMENTED; N2.9-N2.12 PENDING" ||
      phase?.runtimeStatus !== "N2 CANONICAL MEMORY RUNTIME INACTIVE; N2-M1 + N2-M2 + N2-M3 PROMOTED DIAGNOSTICS; N2-M4 SOURCE-IMPLEMENTED DIAGNOSTIC ONLY" ||
      ...
      phase?.phases?.slice(7, 9).some(row => row.status !== "source-implemented" || row.promotedSourceSha !== null || row.builderRunNumber !== null) ||
      ...
      phase?.macroImplementationPlan?.[3]?.status !== "source-implemented" ||
